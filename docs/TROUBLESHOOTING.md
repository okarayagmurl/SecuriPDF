...
**Stirling 2.13 bilinen hata:** `/api/v1/convert/url/pdf` async iş parçacığında `No current ServletRequestAttributes` (GitHub #5047). SecuriPDF platformu sayfayı kendisi indirip `/api/v1/convert/html/pdf` ile dönüştürür.
...
3. **500 — ServletRequestAttributes (eski platform):** Stirling log: `ConvertWebsiteToPDF.urlToPdf` — güncel platform sürümünde html/pdf yolu kullanılır.
4. **URL_FETCH_FAILED:** Container hedef URL'ye çıkamıyor veya yanıt HTML değil. `docker exec entera-pdf curl -sI https://example.com`. JS/SPA siteler kısıtlıdır.

**Debug raporu yolu** (`/data` değil):

```bash
docker exec securipdf-platform ls -la /vault-data/debug-reports/
docker exec securipdf-platform cat /vault-data/debug-reports/RPT-XXXXXXXX-XXXXXX.json
```

`publicHint` boşsa Stirling HTML/boş gövde dönmüş olabilir; güncel platform sürümünde HTTP kodu ve `urlInput` rapora eklenir.
