...
print_next_steps() {
  ...
  echo "=============================================="
  echo ""
  echo "  Ilk kurulum: ${INSTALLER_APP_URL}/setup"
  echo "  URL:         ${INSTALLER_APP_URL}"
  echo "  Admin:       ${INSTALLER_APP_URL}/admin"
  echo ""
  echo "  Setup bitene kadar Keycloak girisi kapali"
  echo "  (OAUTH2_SKIP_AUTH_REGEX=^/.*$)."
  echo ""
  echo "  Break-glass (setup sonrasi):"
  echo "    Kullanici: securipdf-local-admin"
  echo "    Parola:    (CREDENTIALS dosyasina bakin)"
  echo ""
  echo "  Sonraki adimlar:"
  echo "    1. http://.../setup — depolama + varsayilan yonetici"
  echo "    2. Admin panel > Active Directory / LDAP"
  echo "    3. Baglanti testi + Keycloak'a uygula"
